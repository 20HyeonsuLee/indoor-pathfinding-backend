...
def main():
    ...
    for fi, fdata in sorted(floors.items()):
        ...
        # Spur 제거
        skel_before = skel.sum()
        skel = remove_spurs(skel, CELL_SIZE)
        print(f"  spur 제거 후: {skel.sum()}개 (제거: {skel_before - skel.sum()}px)")
