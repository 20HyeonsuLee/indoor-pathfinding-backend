...
> 이 문서는 기존 ARCHITECTURE.md의 설계를 보완하는 논의 결과만 정리한다.
> 기존 파이프라인 설계(pose 기반)에서 발견된 문제와 그 해결 방향.

---

### 문제 1: pose 궤적만으로 경로를 만들면 안 되는 이유

좁은 복도에서는 걸은 궤적 ≈ 이동 경로라서 pose만으로 충분하다. 하지만 넓은 공간(로비, 홀)에서는 스캔자가 지그재그로 돌아다니기 때문에, pose 궤적이 곧 네비게이션 경로가 되면 안 된다.

```
좁은 복도: pose 궤적 ≈ 네비게이션 경로 (OK)
넓은 로비: pose 궤적 = 스캔자의 산책 동선 (쓸모없음)
```

RTAB-Map DB에는 pose 외에 rotation, 포인트 클라우드, RGB 이미지가 전부 있다. 이걸 종합해서 **공간 구조 기반으로 노드를 찍어야 한다.**

---

### 결정 1: 바닥 범위 중앙에 노드를 찍는다

#### 왜 벽이 아닌 바닥인가

초기 아이디어는 "양쪽 벽을 감지해서 벽 사이 중앙에 노드"였다. 하지만 통로가 항상 벽에 둘러싸여 있지 않다:

- 로비에서 복도로 자연스럽게 이어지는 구간 (한쪽 벽 없음)
- 기둥만 있는 통로 (벽이 아닌 기둥 사이)
- 한쪽만 벽인 공간 (반대쪽은 열린 로비)
- 계단 앞 공간 (벽 없이 열려있음)

바닥은 항상 존재한다. 벽이 있으면 바닥 끝 = 벽 위치이므로 결과가 같고, 벽이 없으면 바닥이 끊기는 지점이 경계가 된다.

#### 프레임 그룹핑

단일 프레임의 포인트 클라우드는 카메라 시야(FOV) 한계로 양쪽 바닥을 다 못 볼 수 있다. 주변 ±N개 프레임(기본 ±10)의 포인트 클라우드를 합쳐서 분석한다. 스캔 중 카메라를 좌우로 돌리기 때문에 주변 프레임을 합치면 양쪽 정보가 보완된다.

#### 알고리즘

```
1. 주변 ±10 프레임의 포인트 클라우드 합치기
2. 바닥 평면 추출: Z값이 현재 pose Z ± 0.3m 이내
3. 이동 방향 계산 (rotation 행렬 3번째 열 = 전방 벡터)
4. 이동 방향에 수직인 방향으로 바닥 폭 측정
5. 바닥 범위의 중앙점 = 노드 위치
6. 바닥 폭도 기록 (허브 탐지에서 재사용)
```

#### 부수 효과: 왕복 문제 자연 해결

같은 복도를 왕복 스캔해도 바닥 중앙은 동일하므로 노드가 같은 위치에 찍힌다. KD-Tree 중복 제거에서 자연스럽게 하나로 합쳐진다. 기존의 merge_overlapping_segments(재방문 감지) 로직이 불필요해진다.

---

### 결정 2: 모든 프레임에서 일단 노드를 찍고, 허브는 나중에 정리한다

#### 기각된 접근: 사전 분류 후 허브 스킵

"프레임마다 복도인지 허브인지 먼저 판별 → 복도면 노드 찍고 허브면 스킵 → 나중에 허브 진입점 연결"

이 방식의 문제:
- 개별 프레임 시점에서 허브 판별이 부정확 (넓은 복도를 허브로 오분류)
- 판별이 틀리면 처음부터 꼬임 (스킵한 곳이 복도였으면 노드 없음)
- 이미지 분류 모델이 선행 조건이 됨 (모델 없으면 파이프라인이 안 돌아감)

#### 최종 접근: 일단 다 찍고 패턴으로 발견

공간 유형 상관없이 모든 프레임에서 바닥 중앙 노드를 찍는다. 전체 노드가 다 찍힌 후에 밀집+방향 패턴으로 허브를 발견하여 정리한다.

이유:
- 전체 노드 맵을 보고 판단 → 개별 프레임보다 정확
- 이미지 분류 모델 없이도 기본 파이프라인 동작
- 허브를 "판별"이 아니라 "발견"하는 것 → 사전 정의 불필요

---

### 결정 3: 허브 = "노드 밀집 + 방향 분산" 영역
...
각 노드 주변 반경 R(3m) 안에서:
1. 노드 밀집도 확인 (근처 노드 N개 이상)
2. 근처 노드들의 이동 방향을 클러스터링 (30° 임계값)
3. 방향 클러스터 3개 이상 → 허브

```
복도:  노드 밀집 O, 방향 유사 (한 방향)       → 허브 아님
교차점: 노드 밀집 O, 방향 2~4개 (T자/십자)   → 허브로 탐지
로비:  노드 밀집 O, 방향 다양 (여러 방향 스캔) → 허브로 탐지
```

교차점과 허브를 별도 로직 없이 같은 메커니즘으로 처리한다.

```python
def find_hub_zones(nodes, radius=3.0, min_directions=3):
    hubs = []
    for node in nodes:
        nearby = get_nodes_within(nodes, node, radius)
        if len(nearby) < threshold:
            continue
        directions = [n.direction for n in nearby]
        direction_clusters = cluster_angles(directions, angle_threshold=30)
        if len(direction_clusters) >= min_directions:
            hubs.append(node.position)
    return merge_nearby_hubs(hubs)
```

#### 허브 정리

1. **경계 노드 식별**: 허브 안에 있는 노드 중, 이전 또는 다음 프레임의 노드가 허브 밖에 있는 노드 = 진입/출구.

```python
for node in hub_zone.nodes:
    prev = node.previous  # 시간순 이전 프레임
    next = node.next      # 시간순 다음 프레임
    if prev not in hub_zone or next not in hub_zone:
        → 경계 노드 (유지)
```

2. **내부 노드 삭제**: 경계 노드 외 전부 제거.
3. **경계 노드 연결**: 경계 노드끼리 직선 거리로 엣지 연결. 허브 내에서는 어디든 직선 이동 가능하므로.

결과: 스캔자가 로비에서 아무리 지그재그로 걸어도, 최종 그래프에는 진입/출구 노드와 그 사이 직선 엣지만 남는다.
...
| 엣지케이스 | 위험도 | 대응 |
|-----------|--------|------|
| 이동 방향 = 0 (정지/회전) | 높음 | pose 간 이동벡터가 0이면 rotation 행렬의 forward 벡터(3번째 열)로 폴백 |
| 바닥 미감지 (카메라가 위/벽만 봄) | 높음 | ±10 프레임 합산 후에도 바닥 포인트 부족 시 pose 위치로 폴백. 연속 실패 시 이전 프레임 방향 이어쓰기 |
| 계단/경사로 (Z ± 0.3m 필터에 걸림) | 중간 | 계단은 기존 vertical_detector로 먼저 분리 → 바닥 중앙 로직 적용 안 함 |
| 바닥 위 장애물 (의자, 테이블) | 낮음 | Z ± 0.3m 필터로 대부분 배제됨. 낮은 물체는 바닥 범위에 포함돼도 중앙 계산에 영향 미미 |
| 유리/반사 바닥 | 낮음 | 포인트클라우드에서 감지 실패 → 바닥 미감지 케이스로 폴백 |

#### 허브 탐지 단계

| 엣지케이스 | 위험도 | 대응 |
|-----------|--------|------|
| 복도 허브 오탐 (카메라 좌우 회전) | 중간 | 방향 = pose 간 이동 벡터로 한정. rotation(카메라 시선)이 아니라 실제 이동 방향 사용 |
| L자 코너 → 허브 오탐 | 낮음 | 방향 클러스터 2개 → min_directions=3 미만이므로 허브 아님. 노이즈로 3개 잡히는 건 angle_threshold=30°가 방지 |
| 허브 반경 3m의 범용성 | 중간 | 바닥 폭 정보를 활용한 적응형 반경 검토. 1차 구현은 3m 고정 후 실데이터 검증 |

#### 허브 정리 단계

| 엣지케이스 | 위험도 | 대응 |
|-----------|--------|------|
| 경계 노드 없음 (스캔 시작/끝이 허브 안) | 높음 | 스캔 시작/끝 프레임은 무조건 경계 노드로 처리 |
| 인접 허브 경계 충돌 | 중간 | 허브 병합 단계에서 반경 내 허브끼리 합치기 → 내부에 있던 경계 노드는 병합된 허브의 내부가 되어 자연 해결 |
| 허브 내 모든 노드 삭제 후 고립 | 낮음 | 최소 2개 경계 노드 보장. 1개 이하면 허브로 처리하지 않음 |
...
│
├─ ① 데이터 추출
│    pose(위치+방향) + 포인트 클라우드 + RGB 이미지
│
├─ ② 바닥 중앙 노드 생성 (모든 프레임)
│    포인트 클라우드 바닥 범위 → 중앙점에 노드
│    공간 유형 불문, 일단 전부 찍음
│
├─ ③ 전처리
│    이상치 제거 (중앙값 + MAD)
...
├─ ④ 허브 탐지 및 정리
│    노드 밀집 + 방향 분산 → 허브 발견
│    내부 노드 삭제, 경계 노드만 유지, 상호 연결
│
├─ ⑤ 복도 구간 단순화 (허브 밖만)
│    PCA 직선화 + RDP + KD-Tree 중복 제거
│
└─ ⑦ 최종 그래프 조립 → JSON → Spring Boot
```
...
| 결정 | 기각된 대안 | 최종 선택 | 이유 |
|------|-----------|----------|------|
| 노드 위치 기준 | 벽 사이 중앙 | 바닥 범위 중앙 | 벽 없는 통로에서도 동작 |
| 허브 처리 | 사전 분류 후 스킵 | 일단 다 찍고 후처리 | 전체 정보로 판단, 모델 비의존 |
| 허브 판별 | 이미지 분류 / 바닥 폭 임계값 | 밀집 + 방향 분산 | 교차점도 동시 처리, 자동 발견 |
| 재방문 처리 | merge_overlapping 방향 보강 | 바닥 중앙 수렴 + KD-Tree | 별도 로직 불필요 |
| 포인트 클라우드 시야 | 단일 프레임 | ±10 프레임 그룹핑 | 양쪽 바닥 범위 확보 |
